...
First off, since there are about $$16000$$ vertices here, if the points were evenly spread amongst them then each vertex would correspond to $$1/16000$$ of the data (which is about $$0.006\ \%$$ of the data; or, 4 to 5 instances at each vertex). The data is not that spread out &ndash; by getting a random row and looking at the vertex corresponding to it, it seems typical to find a vertex with something between $$0.5\ \%$$ and $$2\ \%$$ of the data<d-footnote>There are likely numerous vertices with no corresponding data instances, I have not checked.</d-footnote> 
