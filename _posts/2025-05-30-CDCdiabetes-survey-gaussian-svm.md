...
The hyperparameter $$\gamma$$ is the one that appears in the kernel $$K(x,x')$$ of the last section. Considering how $$K$$ is a Gaussian function, in the same form as the distribution of a normally-distributed random variable, $$\gamma$$ would be inversely proportional to the variance.  Hence, the Gaussian summands $$K(x,x_{i_j})$$ that are centered at a support vector have smaller variance when $$\gamma$$ is increased (_variance_ here being the $$\sigma^2$$ of the random variable, not Variance as in the Bias-Variance trade-off).  As a consequence, increasing $$\gamma$$ results in a model which may more easily overfit the data.<d-footnote>That is, increasing $\gamma$ produces a **model** Variance that is higher &ndash; here it _does_ mean Variance from the Bias-Variance trade-off.</d-footnote>

The hyperparameter $$C$$ relates to weight regularization of the model.  Specifically, for a set of weights $$\theta$$, the loss or objective function will be set equal to $$\frac12\lVert\theta\rVert^2 + C\mathcal L_{train}(\theta)$$, with $$\mathcal L_{train}$$ being the standard (or, non-regularized) loss function.  For example, here we have a two-label classification. Using $$f_{\theta}(x)$$ for the number $$\Sigma_{j=1}^k\alpha_j K(x, x_{i_j})$$<d-footnote>This is the ubiquitous affine function $v\mapsto Wv+b$ being applied to the image $\Phi(x)$ in $\mathbb H$.</d-footnote> and $$\sigma$$ for the logistic function, the standard objective in this setting is the log-loss (or binary cross-entropy).  Given a sample training point $$x_i$$, the per-example loss of this function is $$-\log(\sigma(f_{\theta}(x_i)))$$ if $$y_i$$ is positive and $$-\log(\sigma(1 - f_{\theta}(x_i)))$$ if $$y_i$$ is negative.

An increase in $$C$$ causes $$\mathcal L_{train}(\theta)$$ to have an increased effect on the values of the regularized objective function.  That means that updates in weights will be more targeted to bring down the value of $$\mathcal L_{train}(\theta)$$ (which is lower when the model labels training data better, on average), perhaps at the expensive of large weights (trainable parameters).  The tendency to do this is the classical cause of overfitting to the data. So, increases in $$C$$ will make the model more likely to overfit.
...
Keeping these observations in mind, I chose an increasing range of values for $$\gamma$$ between 1 and 1.5 (the value of $$1/(n_features*variance)$$ is in that range) and a decreasing range of values of $$C$$, from 1.5 down to 0.5. (I did not make the mesh very fine, for reasons of computational expense.)  Specifically, I did a grid search with 5-fold cross-validation, my pairs $$(\gamma, C)$$ being chosen so that $$\gamma \in \{1.15, 1.2, 1.25, 1.3, 1.35, 1.4, 1.45, 1.5\}$$ and $$C \in \{1.5, 1.3, 1.1, 0.9, 0.7, 0.5\}$$.  The best performer was $$(\gamma, C) = (1.5, 1.1)$$.
